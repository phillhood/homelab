...
verify: ## Read-only health probes against the real systems
	...
	@printf "%-32s %s/13\n" "infra dns records"   "$$(for n in router switch ap proxmox kvatch pihole music forge registry proxy pault headscale pault-staging; do dig +short @$(PIHOLE_IP) $$n.home; done | grep -c '^192')"
	...
	@printf "%-32s %s\n" "pault postgres"        "$$(ssh -o BatchMode=yes root@$(PAULT_IP) 'systemctl is-active postgresql')"
	@printf "%-32s %s\n" "pault postgres tcp shut" "$$(ssh -o BatchMode=yes root@$(PAULT_IP) 'ss -lnt | grep -q :5432 && echo OPEN-BAD || echo closed')"
	...
	@printf "%-32s %s\n" "pault api healthz"     "$$(curl -s -o /dev/null -w '%{http_code}' https://$(PAULT_HOST)/healthz)"
	...
	@printf "%-32s %s\n" "pault-staging minio"   "$$(ssh -o BatchMode=yes root@$(PAULT_STAGING_IP) 'docker inspect -f "{{.State.Status}}" pault-minio 2>/dev/null || echo MISSING')"
	@printf "%-32s %s\n" "pault-staging 9001 shut" "$$(ssh -o BatchMode=yes root@$(PAULT_STAGING_IP) 'ss -lnt | grep -q :9001 && echo OPEN-BAD || echo closed')"
	@printf "%-32s %s\n" "pault-staging bucket"  "$$(curl -s -o /dev/null -w '%{http_code}' http://$(PAULT_STAGING_IP):9000/minio/health/live)"
	@printf "%-32s %s\n" "pault-staging postgres" "$$(ssh -o BatchMode=yes root@$(PAULT_STAGING_IP) 'systemctl is-active postgresql')"
	@printf "%-32s %s\n" "pault-staging postgres tcp shut" "$$(ssh -o BatchMode=yes root@$(PAULT_STAGING_IP) 'ss -lnt | grep -q :5432 && echo OPEN-BAD || echo closed')"
	@printf "%-32s %s\n" "pault-staging no tunnel" "$$(ssh -o BatchMode=yes root@$(PAULT_STAGING_IP) 'docker inspect pault-cloudflared >/dev/null 2>&1 && echo PRESENT-BAD || echo absent')"
	@printf "%-32s %s\n" "pault-staging web"     "$$(curl -s https://pault-staging.$(LAB_DOMAIN)/ | head -c 20)"
	@printf "%-32s %s\n" "pault-staging api healthz" "$$(curl -s -o /dev/null -w '%{http_code}' https://pault-staging.$(LAB_DOMAIN)/healthz)"
	@printf "%-32s %s\n" "pault-staging media"   "$$(curl -s -o /dev/null -w '%{http_code}' https://pault-staging-media.$(LAB_DOMAIN)/minio/health/live)"
	@printf "%-32s %s\n" "pault-staging web digest" "$$(ssh -o BatchMode=yes root@$(PAULT_STAGING_IP) 'docker image inspect -f "{{index .RepoDigests 0}}" $$(docker inspect -f "{{.Image}}" pault-web) 2>/dev/null || echo gated' | tail -1)"
	@printf "%-32s %s\n" "pault-staging api digest" "$$(ssh -o BatchMode=yes root@$(PAULT_STAGING_IP) 'docker image inspect -f "{{index .RepoDigests 0}}" $$(docker inspect -f "{{.Image}}" pault-api) 2>/dev/null || echo gated' | tail -1)"
